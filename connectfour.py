...
def generate_segments(
    num_columns: int, num_rows: int, segment_legth: int
) -> List[List[Tuple[int, int]]]:
    segments: List[List[Tuple[int, int]]] = []
    # generate the vertical segments
    for c in range(num_columns):
        for r in range(num_rows - segment_legth + 1):
            segment: List[Tuple[int, int]] = []
            for t in range(segment_legth):
                segment.append((c, r + t))
            segments.append(segment)

    # generate the horizontal segments
    for c in range(num_columns - segment_legth + 1):
    ...
    for c in range(num_columns - segment_legth + 1):
        for r in range(segment_legth - 1, num_rows):
            segment = []
            for t in range(segment_legth):
                segment.append((c + t, r - t))
            segments.append(segment)
...
class C4Board(Board):
    ...
    def _evaluate_segment(
        self, segment: List[Tuple[int, int]], player: Piece
    ) -> float:
        black_count, red_count = self._count_segment(segment)
